...
def main():
    stc.html(html_temp)
    menu = ["Home", "Machine Learning App"] # Bisa ditambahkan sendiri misal menu EDA
    ...
    if choice == "Home":
        ...
        run_ml_app()
    # Kalo ada tambahan menu, jangan lupa kasih "alternate path": tambah elif
...
def predict(gender, married, dependent, education, self_employed, applicant_income, coApplicant_income
    ...
    # Preprocessing User Input (Label Encoding)
    gen = 0 if gender == "Male" else 1
    mar = 0 if married == "Yes" else 1
    edu = 0 if education == "Graduate" else 1
    sem = 0 if self_employed == "Yes" else 1
    pro = 0 if property_area == "Semiurban" else 1 if property_area == "Urban" else 2 # if property_area == "Rural" else 3
